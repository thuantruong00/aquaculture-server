...
function handleTurnOff(data, limitTemp) {
  ...
  // const humidity = $(`#value-humidity-${idDevice}`).text();
  // const ph = $(`#value-ph-${idDevice}`).text();
  // const tds = $(`#value-tds-${idDevice}`).text();
  // const nitro = $(`#value-nitro-${idDevice}`).text();
  // const phos = $(`#value-phos-${idDevice}`).text();
  // const pota = $(`#value-pota-${idDevice}`).text();
...
function handleControl(type) {
  ...
  const btns = document.querySelectorAll(".btn-control-port");
  const ports = document.querySelectorAll(".wrapper-port");

  localStorage.setItem("status-control", type);
  const statusControl = localStorage.getItem("status-control");
  console.log(statusControl);

  if (statusControl === "auto") {
    if (elements) {
      elements.forEach(function (element) {
    ...
    $(".btn-save-all").addClass("d-none");
    $(".btn-manual").removeClass("btn-primary");
    $(".btn-auto").addClass("btn-primary");
  } else if (statusControl === "manual") {
    if (elements) {
      elements.forEach(function (element) {
    ...
    $(".btn-save-all").removeClass("d-none");

    $(".btn-manual").addClass("btn-primary");
...
function handleEditPort(e) {
  ...
  $(`.${e}`).each(function (index, element) {
    let name = $(`.ip-name-${e}-${index}`).val();
    let order = index;
    let group = $(`.ip-group-${e}-${index}`).val();
  ...
  if (hasDuplicate(arrayGroup)) {
    $(".modal-id").append(modal("Group is not duplicate"));
    setTimeout(() => {
      handleCloseModal();
    }, 1500);
    return;
...
function handleAddNewDevice() {
  ...
  if (!type || idDevice === "") {
    $(".modal-id").append(modal("Input is required"));
...
function handleDecrease(i, order, idPort) {
  if (Number($(`.${i}-${idPort}-${order}`).val()) >= 1) {
    const val = Number($(`.${i}-${idPort}-${order}`).val()) - 1;
    $(`.${i}-${idPort}-${order}`).val(val);
    $(`.value-brightness-${i}`).text(val);
    command(idPort, order);
  }
}

function handleOnChangeInput(order, idPort) {
  command(idPort, order);
}

function handleIncrease(i, order, idPort) {
  if (Number($(`.${i}-${idPort}-${order}`).val()) <= 99) {
    const val = Number($(`.${i}-${idPort}-${order}`).val()) + 1;
    $(`.${i}-${idPort}-${order}`).val(val);
    $(`.value-brightness-${i}`).text(val);
    command(idPort, order);
...
function savePort() {
  ...
  if (elements) {
    elements.forEach(function (element) {
      ...
      let port1 = $(`.port1-${idDevice}-${order}`).val();
      let port2 = $(`.port2-${idDevice}-${order}`).val();
...
function command(idDevice, order) {
  let p1 = $(`.port1-${idDevice}-${order}`).val();
  let p2 = $(`.port2-${idDevice}-${order}`).val();
...
function handleExportData() {
  const fromDate = $(`#from-date`).val();
  const timestampFromDate = new Date(fromDate).getTime();

  const toDate = $(`#to-date`).val();
  const timestampToDate = new Date(toDate).getTime();
  ...
  if (timestampFromDate > timestampToDate) {
    alert("End date must bigger than start date.");

    return;
  }

  const data = { timestampFromDate, timestampToDate, arrayIdPort };
  console.log(data);
...
function handleChoseIdDevice(value, portArray) {
  ...
  const template = filter.map((item) => {
    return `
    <option name=${item.name.replace(" ", "-")} value=${item.order} id="element-name">${item.name}</option>`;
  });
...
function handleSaveTemp(idDevice) {
  const name = $(`#ip-name-${idDevice}`).val();
  const group = $(`#ip-group-${idDevice}`).val();
  const data = { name, group, idDevice, status: "info" };

  if (data.group < 0 || data.group > 7) {
    $(".modal-id").append(modal("Group required 0 - 7"));
    setTimeout(() => {
      handleCloseModal();
    }, 1500);
    setTimeout(() => {
...
function submitTimer() {
  ...
  const valueOrder = $("#name-order").val();
  const valueName = $("#name-order").find(":selected").text();
...
function handleChangeStatus(_id) {
  const status = $(`.id-status-${_id}`).text();
  let data = {};
  if (status === "active") {
  ...
  $.ajax({
    ...
    data,
    success: function (result) {
      $(`.id-status-${_id}`).text(result.status);
    },
  });
}
